...
- The grid has an optional **Summer** column, but summer offerings are **not
  modelled**. UCSB Summer Sessions publish a separate, much smaller catalog that none
  of the sources behind this app cover, so a summer placement is never checked — it
  always warns and tells you to confirm in GOLD. Prerequisite ordering *is* enforced
  in Summer, since that only depends on the term's position.
- Offerings are a *plan*, not a guarantee. The ECE department says its grid is
  subject to change; confirm in
  [GOLD](https://my.sa.ucsb.edu/gold/login.aspx) before registering.
- Prerequisites are enforced as course ordering only. Minimum grades and major
  restrictions are displayed, not checked — the app does not know your grades.
- Data is a snapshot: the 2025-26 General Catalog, the 2026-27 ECE grid and the
  2026-27 GEAR. Re-run the pipeline when UCSB publishes new ones.
- The degree audit covers **major** requirements only. General education, writing,
  the American History and Institutions requirement and free electives are not
  modelled, so planned units always read short of the total needed to graduate.
  It is a planning aid, not a degree check — GOLD's progress check and your adviser
  are the authority.
- `CMPSC 41`, required for CE from 2026-27, is not in the published catalog yet, so
  it carries no prerequisites or quarters here. UCSB has not released a 2026-27
  General Catalog.
- Elective totals ignore `ECE 194` special topics, which do count towards the major
  but have placeholder catalog titles and so are not listed.
- Plans, prior credit and waivers are saved in browser `localStorage`. There are no
  user accounts, so they do not follow you to another machine.
- A waiver suppresses the prerequisite check but proves nothing. UCSB still enforces
  prerequisites at registration; get the exception from your adviser.

## Roadmap

- [x] Check a plan against the EE track / CE sequence requirements
- [ ] Prerequisite arrows drawn between placed courses
- [ ] Filter the sidebar by track, and suggest the courses a track still needs
- [ ] Model general education so the unit total is meaningful
- [ ] Save plans server-side, with accounts
- [ ] Favourites
- [ ] Multiple plan drafts
- [ ] Advisor view with comments
